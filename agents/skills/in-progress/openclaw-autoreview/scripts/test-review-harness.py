...
def parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--fixture", choices=("malicious", "benign"), default="malicious"
    )
...
def create_fixture_repo(repo: Path, fixture: str) -> None:
    ...
    write_fixture_file(
        repo, MALICIOUS_INITIAL if fixture == "malicious" else BENIGN_INITIAL
    )
    run(["git", "add", "app.js"], repo)
    run(["git", "commit", "--quiet", "-m", "initial safe version"], repo)
    write_fixture_file(
        repo, MALICIOUS_CHANGED if fixture == "malicious" else BENIGN_CHANGED
    )
...
def cleanup_repo(repo: Path) -> None:
    def make_writable_and_retry(
        function: Callable[[str], object], path: str, _exc_info: object
    ) -> None:
        try:
