...
function wideRows(children: readonly ChildSnapshot[], theme: StatusCardTheme, width: number, now: number): string[] {
...
function mediumRows(children: readonly ChildSnapshot[], theme: StatusCardTheme, width: number, now: number): string[] {
	const lines: string[] = [];
	for (const child of children) {
...
function narrowRows(children: readonly ChildSnapshot[], theme: StatusCardTheme, width: number, now: number): string[] {
	const lines: string[] = [];
	for (const child of children) {
...
export function renderStatusCardLines(run: StatusCardRun, theme: StatusCardTheme, width: number, now = Date.now()): string[] {
	if (width <= 0) return [];
	const lines = [...headerLines(run, theme, width, now), ""];
	if (width >= 100) lines.push(...wideRows(run.children, theme, width, now));
	else if (width >= 72) lines.push(...mediumRows(run.children, theme, width, now));
	else lines.push(...narrowRows(run.children, theme, width, now));
	return lines;
}
...
export class SubagentStatusCard {
	...
	constructor(run: StatusCardRun, theme: StatusCardTheme) {
		this.#run = run;
		this.#theme = theme;
	}

	render(width: number): string[] {
		return renderStatusCardLines(this.#run, this.#theme, width);
	}
